...
public class ServiceResponseVerifier {
    ...
    public static void verifyExpectedResponseRegister(MockHttpServletResponse response, HttpStatus expectedStatus, ServiceDetailsDto expectedBody) throws Exception {
    ...
    private static void verifyBody(JsonNode rootNode, ServiceDetailsDto expectedBody) throws JsonMappingException, JsonProcessingException {	
	...
	public static void verifyErrorResponseGetById(MockHttpServletResponse response, HttpStatus expectedStatus,
			ServiceDetailsDto expectedBody) throws Exception {
		verifyStatus(response.getStatus(), expectedStatus);
		var body = response.getContentAsString(StandardCharsets.UTF_8);
		JsonNode rootNode = mapper.readTree(body);
		verifyBody(rootNode, expectedBody);
